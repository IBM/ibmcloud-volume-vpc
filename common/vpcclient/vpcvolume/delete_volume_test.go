...
func TestDeleteVolume(t *testing.T) {
	...
	for _, testcase := range testCases {
		t.Run(testcase.name, func(t *testing.T) {
			...
			test.SetupMuxResponse(t, mux, vpcvolume.Version+"/volumes/volume-id", http.MethodDelete, nil, testcase.status, testcase.content, nil)
			...
			err := volumeService.DeleteVolume("volume-id", logger)
